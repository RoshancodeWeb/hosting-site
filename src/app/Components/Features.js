...
const IndividualFeatures = ({ imageUrl,title,description }) => {
  return (
    ...
    <div className='w-[80vw] lg:w-[50%] h-[300px] pt-3 pl-3  group hover:shadow-lg  transition-shadow duration-300   hover:shadow-gray-600 cursor-pointer  bg-[#FFFFFF] rounded-xl'>
      ...
      <Image className='cursor-pointer' src={imageUrl} width={60} height={60} alt='feature-image'/>
    ...
    <div className='w-full min-h-fit bg-[#F5EDE9]'>
      ...
      <div className='w-full min-h-[50vh] bg-[#F5EDE9] mt-5  pl-8 lg:pb-5 lg:pl-2 lg:pr-2 flex flex-col pr-10 lg:flex-row'>
          <div className='w-full  h-fit flex flex-col gap-5 md:flex-row lg:w-[50%] lg:pr-2'>
             <IndividualFeatures  imageUrl="/menu_one_new.png" title="WordPress Hosting" description="Bring your WordPress site to the new age with the speed and security you expect today, backed up by expert support." />
             <IndividualFeatures  imageUrl="/menu_two_new.png" title="Static Site Hosting" description="Deploy your static website or the prebuilt elements of a composable architecture to a global edge network." />
          </div>

          <div className='w-full  h-fit flex flex-col gap-5 pt-5 lg:pt-0 md:flex-row lg:w-[50%]'>
             <IndividualFeatures  imageUrl="/menu_three_new.png" title="Web App Hosting" description="Run any code from frontend to backend, and don’t worry about the infrastructure, with automatic deployments." />
             <IndividualFeatures  imageUrl="/menu_four_new.png" title="Database Hosting" description="Manage your databases where they are not the afterthought, tuned for speed with real dedicated resources." />
          </div>
